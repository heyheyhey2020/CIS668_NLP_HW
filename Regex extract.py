...
#tp+?
epatterns.append('([A-Za-z.]+)\s*@\s*([A-Za-z.]+)\s*\.\s*[edu|EDU]')

#especially for latombe
epatterns.append('([A-Za-z.]+)<del>\s*@\s*([A-Za-z.]+)\.edu')

#especially for levoy format: ada&#x40;graphics.stanford.edu
epatterns.append('([A-Za-z.]+)\s*&#x40;\s*([A-Za-z.]+)\s*\.\s*edu')

#espacially for manning format: mailto:dbarros <at symbol> cs.stanford.edu
epatterns.append('([A-Za-z.]+)\s*<at symbol>\s*([A-Za-z.]+)\s*\.\s*edu')

#espacially for ouster format : ouster (followed by &ldquo;@cs.stanford.edu&rdquo;)
epatterns.append('([A-Za-z.]+)\s*\(followed\s*by\s*&ldquo;@([A-Za-z.]+)\.edu')
#espacially for ouster format : teresa.lynn (followed by "@stanford.edu")
epatterns.append('([A-Za-z.]+)\s*\(followed\s*by\s*"@\s*([A-Za-z.]+)\.edu')

#espacially for subh format :
epatterns.append('([A-Za-z.]+)\s*AT\s+([A-Za-z.]+)\s*DOT\s+edu')

#espacially for engler format :
epatterns.append('([A-Za-z.]+)\s*WHERE\s*([A-Za-z.]+)\s*DOM\s*edu')

#espacially for vladlen format: vladlen at <!-- die!--> stanford <!-- spam pigs!--> dot <!-- die!--> edu
#epatterns.append('([A-Za-z.]+)\s*at\s*<!-- die!-->\s*([A-Za-z.]+)\s*<!-- spam pigs!-->\s*dot\s*<!-- die!-->\s*edu')
#epatterns.append('vladlen at <!-- die!--> stanford <!-- spam pigs!--> dot <!-- die!--> edu')

#hager: hager at cs dot jhu dot edu
#epatterns.append('([A-Za-z.]+)\s*at\s*([A-Za-z.]+)\s*dot\s*([A-Za-z.]+)\s*dot\s*edu')




#epatterns.append('([A-Za-z.]+)\s< atsymbol >\s([A-Za-z.]+)\.(edu|EDU)')
#epatterns.append('([A-Za-z.]+)\s(@|at)\s([A-Za-z.]+)\.[edu|EDU]')
#tp+2, fp+2, fn-2
#epatterns.append('([A-Za-z.]+)\sat\s([A-Za-z.]+)\.(EDU|edu)')
...
ppatterns.append('\[(\d{3})\]\s*(\d{3})-(\d{4})')
#works
ppatterns.append('\+1\s(\d{3})\s(\d{3})[\s|-](\d{4})')
